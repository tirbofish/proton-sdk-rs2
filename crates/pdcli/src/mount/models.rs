...
impl FsNode {
    ...
    pub fn from_node(node: &Node) -> Self {
        match node {
            ...
            Node::Photo(f) => FsNode::File(ProtonFileMetadata::from_file_node(f, true, None)),
    ...
    pub fn from_degraded(node: &DegradedNode) -> Self {
        let (uid, parent_uid, name, is_file, mime_type, size_on_cloud, creation_time, errors) =
            match node {
                DegradedNode::File(f) | DegradedNode::Photo(f) => {
                    let name = match &f.base.name {
                        ...
                        PotentialObject::Degraded(_) => {
                            format!("[degraded-{}]", f.base.uid.link_id.raw())
                        }
                    };
