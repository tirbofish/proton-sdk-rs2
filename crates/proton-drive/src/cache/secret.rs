...
pub trait DriveSecretCache: Send + Sync {
    async fn set_share_key(&self, share_id: ShareId, share_key: PgpPrivateKey) -> anyhow::Result<()>;
...
impl DriveSecretCache for PhotosSecretCache {
    async fn set_share_key(&self, share_id: ShareId, share_key: PgpPrivateKey) -> anyhow::Result<()> {
        ...
        self.repository.set(&share_key_cache_key(&share_id), serialized, vec![]).await
    }
    ...
    async fn set_folder_secrets(
        ...
        self.repository.set(&folder_secrets_cache_key(&node_id), serialized, vec![]).await
    }
    ...
    async fn try_get_folder_secrets(
        ...
        let value = self.repository.try_get(&folder_secrets_cache_key(&node_id)).await?;
        Ok(match value {
    ...
    async fn set_file_secrets(
        ...
        let serialized = serde_json::to_string(&secrets)?;
        self.repository.set(&file_secrets_cache_key(&node_id), serialized, vec![]).await
    }
...
impl DriveSecretCache for DefaultDriveSecretCache {
    ...
    async fn try_get_share_key(&self, share_id: ShareId) -> anyhow::Result<Option<PgpPrivateKey>> {
        let value = self.repository.try_get(&share_key_cache_key(&share_id)).await?;
        Ok(match value {
            Some(s) => Some(serde_json::from_str(&s)?),
            None => None,
        })
    ...
    async fn try_get_file_secrets(
        ...
        let value = self.repository.try_get(&file_secrets_cache_key(&node_id)).await?;
        Ok(match value {
            Some(s) => Some(serde_json::from_str(&s)?),
            None => None,
        })
