...
impl CacheRepository for FileCacheRepository {
    async fn set(&self, key: &str, value: String, tags: Vec<String>) -> anyhow::Result<()> {
        ...
        for tag in new_tags {
            inner.tag_to_keys.entry(tag).or_default().insert(key.to_string());
        }
        drop(inner);
        self.persist()?;
        Ok(())
    ...
    async fn remove_by_tag(&self, tag: &str) -> anyhow::Result<()> {
        let keys: Vec<String> = {
            let mut inner = self.inner.write();
            inner.tag_to_keys.remove(tag).map(|k| k.into_iter().collect()).unwrap_or_default()
        };
        for key in keys {
    ...
    fn get_by_tags(&self, tags: Vec<String>) -> BoxStream<'_, anyhow::Result<(String, String)>> {
        ...
        let results = {
            ...
            keys_set
                .into_iter()
                .filter_map(|key| inner.entries.get(&key).map(|v| Ok((key.clone(), v.clone()))))
                .collect::<Vec<_>>()
        };
