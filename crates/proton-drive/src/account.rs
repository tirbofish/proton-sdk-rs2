...
pub trait AccountClient: Send + Sync {
    ...
    async fn get_address_primary_private_key(&self, address_id: &AddressId) -> anyhow::Result<PrivateKey>;
    async fn get_address_private_keys(&self, address_id: &AddressId) -> anyhow::Result<Vec<PrivateKey>>;
...
impl AccountClient for AccountClientAdapter {
    ...
    async fn get_address_primary_private_key(&self, address_id: &AddressId) -> anyhow::Result<PrivateKey> {
        self.client.get_address_primary_private_key(address_id.raw()).await
    }
