...
impl DeviceOperations {
    ...
    pub async fn list_devices(client: &ProtonDriveClient) -> anyhow::Result<Vec<Device>> {
        ...
        for raw_info in &raw {
            if let Err(e) =
                crate::share_ops::ShareOperations::get_share(client, raw_info.share_id.clone())
                    .await
            {
                tracing::warn!(
    ...
    pub async fn get_device(client: &ProtonDriveClient, device_id: &str) -> anyhow::Result<Device> {
    ...
    pub async fn create_device(
        ...
        let primary_key_index = usize::try_from(default_addr.primary_key_index).unwrap_or(0);
        ...
        let main_folder =
            crate::node::operations::NodeOperations::get_my_files_folder(client).await?;
        ...
        let locked_node_key = node_key.to_armored_private_key(Some(node_passphrase.as_bytes()))?;
        ...
        if let Err(e) =
            crate::share_ops::ShareOperations::get_share(client, resp.device.share_id.clone()).await
        {
            tracing::warn!(
    ...
    pub async fn rename_device(
        ...
        if has_deprecated_name {
            ...
            if let Err(e) = client
                .api()
                .devices()
                .clear_device_share_name(device_id)
                .await
            {
        ...
        let root_uid = NodeUid::new(raw.device.volume_id.clone(), raw.share.link_id.clone());

        // Rename the root node (rename_node with none for media_type).
        crate::node::operations::NodeOperations::rename(
            client,
            root_uid.clone(),
            new_name.clone(),
            None,
    ...
    // Delete device
    pub async fn delete_device(client: &ProtonDriveClient, device_id: &str) -> anyhow::Result<()> {
